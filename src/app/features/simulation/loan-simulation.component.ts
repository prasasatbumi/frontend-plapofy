              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                @for (p of plafonds(); track p.id) {
                  <div 
                    ...
                    <div class="text-xs text-gray-500 mt-1">{{ p.minAmount | currency:'IDR':'symbol':'1.0-0' }} - {{ p.maxAmount | currency:'IDR':'symbol':'1.0-0' }}</div>
                  </div>
                }
              </div>
            </div>
                ...
                <div class="relative">
                 ...
                 @if (simForm.get('amount')?.invalid && simForm.get('amount')?.touched) {
                  <p class="text-xs text-red-500 mt-1">
                    Amount must be between {{ selectedPlafond()?.minAmount | number }} and {{ selectedPlafond()?.maxAmount | number }}
                  </p>
                }
              </div>
